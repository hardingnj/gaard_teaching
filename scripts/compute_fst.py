...
parser.add_argument("--windowsize", "-w", default=20000, type=int)
...
def main(callset, samplesA, samplesB, window_size):
    ...
    assert len(sa) > 0, "No samples in A"
    if len(sa) < len(samplesA):
        print("Not all samples provided in A were found. Found {0}/{1}".format(len(sa), len(samplesA)))
    assert len(sb) > 0, "No samples in B"
    if len(sb) < len(samplesB):
        print("Not all samples provided in B were found. Found {0}/{1}".format(len(sb), len(samplesB)))
    ...
    for start in window_starts:
        ...
        g = allel.GenotypeChunkedArray(callset["calldata/GT"][loc])
